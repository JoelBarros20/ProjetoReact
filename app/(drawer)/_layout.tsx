...
export default function DrawerLayout() {
  ...
  // Arrays das variáveis do menu lateral para abrir as páginas
  const [showCategories, setShowCategories] = useState(false);
  const [showSubAreas, setShowSubAreas] = useState(false);

  // Variáveis das animações dos acordeões do menu
  ...
  const toggleSubAreas = () => {
    ...
    Animated.timing(animationSubAreas, {
      toValue: finalValue,
      duration: 200,
      useNativeDriver: false,
      easing: Easing.out(Easing.ease),
  ...
  const getCategoriaIcon = (cat: string) => {
    switch (cat) {
      case 'Comerciais':
        return <MaterialIcons name="local-shipping" size={18} color="#fff" />;
      case 'Economico':
        return <MaterialIcons name="attach-money" size={18} color="#fff" />;
      case 'Intermedio':
        return <MaterialIcons name="directions-car" size={18} color="#fff" />;
      case 'Minivan':
        return <MaterialIcons name="airport-shuttle" size={18} color="#fff" />;
      case 'SUV':
        return <MaterialCommunityIcons name="car-lifted-pickup" size={18} color="#fff" />;
      case 'Premium':
        return <MaterialIcons name="workspace-premium" size={18} color="#fff" />;
      case 'Desportivo':
        return <Ionicons name="car-sport" size={18} color="#fff" />;
      default:
        return null;
    }
  };
  ...
  return (
    <SessionWrapper>
              ...
              <View style={{ marginTop: 5, marginLeft: -5 }}>
                ...
                <Animated.View style={{ paddingLeft: 60, overflow: 'hidden', height: animation }}>
                  {categorias.map((cat, index) => (
                    <TouchableOpacity key={index} onPress={() => router.push(categoriaSlugs[cat as keyof typeof categoriaSlugs] as any)}
                ...
                <Animated.View style={{ paddingLeft: 60, overflow: 'hidden', height: animationSubAreas }}>
                  {subareas.map((sub, index) => {
                    ...
                    return (
                      <TouchableOpacity key={index} onPress={() => router.push(subareasSlugs[sub as keyof typeof subareasSlugs] as any)}
                        style={{ paddingVertical: 8, flexDirection: 'row', alignItems: 'center', gap: 6 }} >
                        {icon}
      ...
      >
        ...
        {/* <Drawer.Screen name="subareascliente/clientereservas" options={{ title: 'Minhas Reservas' }} /> */}
      </Drawer>
    </SessionWrapper>
  );
}
