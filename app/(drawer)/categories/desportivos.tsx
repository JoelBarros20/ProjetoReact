...
export default function ComerciaisPage() {
  ...
  const filters = images.filter((img) => {
    ...
    const categoryOk = img.category_name?.toLowerCase() === 'desportivos';
    ...
    fetch(API_ROUTES.VEHICLES)
      ...
      .then((data) => {
        const filtered = data.filter(
          (v: any) =>
            v.category_name?.toLowerCase() === 'desportivos' &&
  ...
  return (
    <View style={styles.ContainerMainPage}>
      <View style={styles.HeaderPage}>
        <SideMenu />
        <Text style={styles.MainTitle}> Viaturas Desportivas </Text>
